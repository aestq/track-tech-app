...
export const EquipmentForm = memo((props: EquipmentFormProps) => {
    const { className, onChangeName, onChangeStockNumber, onChangeStatus, onChangeSpecifications, onChangeRoom, data, error } =
        props
    const isAdmin = useSelector(getUserIsAdmin)

    return (
        <div className={classNames(cls.EquipmentForm, {}, [className])}>
            {error && <Text text={error} theme="error" size="s" />}
            <Input
            ...
            <Input
                ...
                readOnly={!isAdmin}
            />
            <Tabs tabs={items} value={data?.status} onChange={onChangeStatus} readOnly={!isAdmin} label="Статус" />
            <Textarea
            ...
            />
            <Input placeholder="Введите кабинет" onChange={onChangeRoom} value={data?.room} label="Кабинет" />
        </div>
    )
})
