...
const AddRoomForm = (props: AddRoomFormProps) => {
    ...
    return (
        <div className={classNames(cls.AddRoom, {}, [className])}>
            {error && <Text className={cls.error} text={error} size="s" theme="error" />}
            <RoomForm number={number} onChangeNumber={onChangeNumber} onClick={onClickCreate} isLoading={isLoading} />
        </div>
    )
}
